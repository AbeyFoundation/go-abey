...
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb abeydb.Putter) error
}
...
func NewDatabase(db abeydb.Database) Database {
...
func NewDatabaseWithCache(db abeydb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb abeydb.Putter) error {

...
func (c *Console) init(preload []string) error {
	...
	flatten := "var abey = web3.abey; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = abey.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines abey.getBalance(abey.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gabey instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gabey metadata
	fmt.Fprintf(c.printer, "Welcome to the Gabey JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + abey.coinbase);
		console.log("at block: " + abey.blockNumber + " (" + new Date(1000 * abey.getBlock(abey.blockNumber).timestamp) + ")");

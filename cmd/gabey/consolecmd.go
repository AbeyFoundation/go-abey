...
// localConsole starts a new gabey node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Failed to attach to the inproc gabey: %v", err)
	}
...
// remoteConsole will connect to a remote gabey instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	// Attach to a remotely running gabey instance and start the JavaScript console
	...
	if endpoint == "" {
		...
		if path != "" {
			...
			} else if ctx.GlobalBool(utils.MainnetClassicFlag.Name) {
				path = filepath.Join(path, "classic")
			} else if ctx.GlobalBool(utils.TestnetClassicFlag.Name) {
				path = filepath.Join(path, "testnet-classic")
			}
		}
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gabey: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.GlobalString(utils.JSpathFlag.Name),
...
// for "gabey attach" and "gabey monitor" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gabey < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
...
// ephemeralConsole starts a new gabey node, attaches an ephemeral JavaScript

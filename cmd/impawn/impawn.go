...
func sendContractTransaction(client *abeyclient.Client, from, toAddress common.Address, value *big.Int, privateKey *ecdsa.PrivateKey, input []byte) common.Hash {
...
func queryTx(conn *abeyclient.Client, txHash common.Hash, contract bool, pending bool, delegate bool) {
...
func queryRewardInfo(conn *abeyclient.Client, number uint64, start bool) {
...
func queryStakingInfo(conn *abeyclient.Client, query bool, delegate bool) {
	header, err := conn.HeaderByNumber(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}

...
import (
	...
	"github.com/AbeyFoundation/go-abey/miner"
...
// PublicAbeychainAPI provides an API to access Abeychain full node-related
// information.
...
// PrivateAdminAPI is the collection of Abeychain full node-related APIs
...
// admin methods of the Abeychain service.
func NewPrivateAdminAPI(abey *Abeychain) *PrivateAdminAPI {
	return &PrivateAdminAPI{abey: abey}
}
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.abey.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.abey.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.abey.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Abeychain full node APIs exposed
...
// related public debug methods of the Abeychain service.
...
// private debug methods of the Abeychain service.
func NewPrivateDebugAPI(config *params.ChainConfig, abey *Abeychain) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, abey: abey}
}

// Preimage is a debug API function that returns the preimage for a sha3 hash, if known.
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.abey.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	blocks := api.abey.BlockChain().BadBlocks()
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.abey.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.abey.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.abey.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.abey.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.abey.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	oldTrie, err := trie.NewSecure(startBlock.Root(), trie.NewDatabase(api.abey.chainDb), 0)
	if err != nil {
		return nil, err
	}
	newTrie, err := trie.NewSecure(endBlock.Root(), trie.NewDatabase(api.abey.chainDb), 0)
	if err != nil {
		return nil, err
	}

...
import (
	...
	elect "github.com/AbeyFoundation/go-abey/consensus/election"
...
var (
	oldReceivedMetrics       = metrics.NewRegisteredMeter("abey/pbftAgent/old", nil)
	repeatReceivedMetrics    = metrics.NewRegisteredMeter("abey/pbftAgent/repeat", nil)
	newReceivedMetrics       = metrics.NewRegisteredMeter("abey/pbftAgent/new", nil)
	differentReceivedMetrics = metrics.NewRegisteredMeter("abey/pbftAgent/different", nil)
	nodeHandleMetrics        = metrics.NewRegisteredMeter("abey/pbftAgent/handle", nil)
...
func NewPbftAgent(abey Backend, config *params.ChainConfig, engine consensus.Engine, election *elect.Election, gasFloor, gasCeil uint64) *PbftAgent {
	agent := &PbftAgent{
		...
		snailChain:           abey.SnailBlockChain(),
		...
		vmConfig:             vm.Config{EnablePreimageRecording: abey.Config().EnablePreimageRecording},
		gasFloor:             gasFloor,
		gasCeil:              gasCeil,
...
func (agent *PbftAgent) initNodeInfo(abey Backend) {
...
func (agent *PbftAgent) validateBlockSpace(header *types.Header) error {
	...
	if snailBlock.Number().Cmp(agent.config.TIP9.SnailNumber) < 0 {
...
func (agent *PbftAgent) rewardSnailBlock(header *types.Header) {
	...
	if space >= params.SnailRewardInterval.Int64() && agent.config.TIP9.SnailNumber.Cmp(reward0) >= 0 {
...
func (env *AgentWork) commitTransactions(mux *event.TypeMux, txs *types.TransactionsByPriceAndNonce, bc *core.BlockChain, feeAmount *big.Int) {
	...
	for {
		...
		txhash := tx.HashOld()
		if env.config.IsTIP10(env.header.Number) {
			txhash = tx.Hash()
		}
		// Start executing the transaction
		env.state.Prepare(txhash, common.Hash{}, env.tcount)

...
func (pm *ProtocolManager) removePeer(id string, call uint32) {
	...
	log.Debug("Removing Abeychain peer", "peer", id, "recipients", len(pm.peers.peers))

	// Unregister the peer from the downloader and Abeychain peer set
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Abeychain protocol stopped")
}
...
func resolveVersionFromName(name string) bool {
	str := name
	flag := "Gabey/v0.8.2"
	if !strings.Contains(str, "Gabey/v0.8") {
		return true
	}
...
// handle is the callback invoked to manage the life cycle of an abey peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Abeychain peer connected", "name", p.Name(), "remoteAddr", p.RemoteAddr())

	// Execute the Abeychain handshake
	var (
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Abeychain peer registration failed", "err", err)
		return err
	}
	...
	if err := pm.downloader.RegisterPeer(p.id, p.version, p.RemoteAddr().String(), p); err != nil {
		p.Log().Error("Abeychain downloader registerPeer registration failed", "err", err)
		return err
	}

	if err := pm.fdownloader.RegisterPeer(p.id, p.version, p); err != nil {
		p.Log().Error("Abeychain fdownloader registerPeer registration failed", "err", err)
		return err
	}
	...
	for {
		err := pm.handleMsg(p)
		if err != nil {
			p.Log().Info("Abeychain message handling failed", "remoteAddr", p.RemoteAddr(), "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Abeychain sub-protocol metadata

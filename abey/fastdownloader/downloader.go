...
type SDownloader interface {
	SyncStateFd(root common.Hash) abey.StateSyncInter
}
...
func New(mode SyncMode, stateDb abeydb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer abey.PeerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	...
	dl := &Downloader{
		...
		receiptCh:     make(chan abey.DataPack, 1),
...
func (d *Downloader) SetSync(StateSync abey.StateSyncInter) {
...
func (d *Downloader) RegisterPeer(id string, version int, peer abey.Peer) error {
...
func (d *Downloader) RegisterLightPeer(id string, version int, peer abey.LightPeer) error {
...
func (d *Downloader) synchronise(id string, hash common.Hash, mode SyncMode, origin uint64, height uint64) error {
	...
	for _, ch := range []chan abey.DataPack{d.headerCh, d.bodyCh, d.receiptCh} {
	...
	p := d.peers.Peer(id)
	if p == nil {
		log.Debug("Fast synchronise,get peer connection err", "id", id)
...
func (d *Downloader) syncWithPeer(p abey.PeerConnection, hash common.Hash, origin uint64, height uint64) (err error) {
...
func (d *Downloader) fetchHeaders(p abey.PeerConnection, from uint64, height int, pivot uint64) error {
...
func (d *Downloader) fetchReceipts(from uint64) error {
	...
	var (
		...
		fetch    = func(p abey.PeerConnection, req *abey.FetchRequest) error { return p.FetchReceipts(req) }
		capacity = func(p abey.PeerConnection) int { return p.ReceiptCapacity(d.requestRTT()) }
		setIdle  = func(p abey.PeerConnection, accepted int) { p.SetReceiptsIdle(accepted) }
	)
...
func (d *Downloader) fetchParts(errCancel error, deliveryCh chan abey.DataPack, deliver func(abey.DataPack) (int, error), wakeCh chan bool,
	...
	fetchHook func([]*types.Header), fetch func(abey.PeerConnection, *abey.FetchRequest) error, cancel func(*abey.FetchRequest), capacity func(abey.PeerConnection) int,
...
func (d *Downloader) importBlockResults(results []*abey.FetchResult) error {
...
func splitAroundPivot(pivot uint64, results []*abey.FetchResult) (p *abey.FetchResult, before, after []*abey.FetchResult) {
...
func (d *Downloader) commitPivotBlock(result *abey.FetchResult) error {

...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Abeychain
...
func (abey *Abeychain) startBloomHandlers() {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-abey.shutdownChan:
					return
				...
				case request := <-abey.bloomRequests:
					...
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(abey.chainDb, (section+1)*params.BloomBitsBlocks-1)
						if compVector, err := rawdb.ReadBloomBits(abey.chainDb, task.Bit, section, head); err == nil {
...
// for the Abeychain header bloom filters, permitting blazing fast filtering.

...
func cancel(conn *abeyclient.Client, value *big.Int) error {
...
func withdrawImpawn(conn *abeyclient.Client, value *big.Int) (common.Hash, error) {
...
func delegateImpawn(conn *abeyclient.Client, value *big.Int, address common.Address, key *ecdsa.PrivateKey) (common.Hash, error) {
...
func cancelDImpawn(conn *abeyclient.Client, value *big.Int, address common.Address, key *ecdsa.PrivateKey) error {
...
func withdrawDImpawn(conn *abeyclient.Client, value *big.Int, address common.Address, key *ecdsa.PrivateKey) (common.Hash, error) {
...
func filterLogs(client *abeyclient.Client) {
	method := "Deposit"
...
func queryRewardImpawn(ctx *cli.Context) error {
	...
	err = client.Call(&head, "abey_getSnailBlockByNumber", "latest", false)
	if err != nil {
		fmt.Println("abey_getSnailBlockByNumber:", err.Error())
		return err
	}
	...
	for i := start; i < head.Number.ToInt().Uint64()-14; i++ {
		...
		err = client.Call(&reward, "abey_getChainRewardContent", hexutil.EncodeBig(new(big.Int).SetUint64(i)), address)
		if err != nil {
			fmt.Println("abey_getChainRewardContent:", err.Error())
			return err
		}

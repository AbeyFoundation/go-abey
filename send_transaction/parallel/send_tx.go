...
func loop(conn *abeyclient.Client, ctx *cli.Context) {
	loopMutex.Lock()
...
func startDelegateTx(conn *abeyclient.Client) {
...
func initAccount(conn *abeyclient.Client) {
...
func sendOtherContractTransaction(client *abeyclient.Client, toAddress common.Address, value *big.Int, privateKey *ecdsa.PrivateKey, nonceOther uint64, wait *sync.WaitGroup) (common.Hash, error) {
	defer wait.Done()
...
func sendSonTransaction(client *abeyclient.Client, from, toAddress common.Address, value *big.Int, privateKey *ecdsa.PrivateKey, input []byte, method string) (common.Hash, error) {
	blockMutex.Lock()
...
func querySendTx(conn *abeyclient.Client) {

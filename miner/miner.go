...
func New(abeychain Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine,
	...
	miner := &Miner{
		...
		worker:            newWorker(config, engine, common.Address{}, abeychain, mux),
	...
	if !remoteMining {
		miner.Register(NewCPUAgent(abeychain.SnailBlockChain(), engine))
		if !miner.singleNode {

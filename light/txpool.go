...
// to the abey network. The implementations of the functions should be non-blocking.
//
...
func (pool *TxPool) checkMinedTxs(ctx context.Context, hash common.Hash, number uint64, txc txStateChanges) error {
	...
	if list != nil {
		...
		rawdb.WriteTxLookupEntries2(pool.chainDb, block, pool.config.TIP10.FastNumber)

...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gabey is "gabey". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gabey", keep that.
	if name == "gabey" || name == "gabey-testnet" {
		name = "Gabey"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gabey" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gabey 1.4 are used if they exist.
	if c.name() == "gabey" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "gabey" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "gabey-keystore")
		ephemeral = keydir
	}

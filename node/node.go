...
func (n *Node) OpenDatabase(name string, cache, handles int) (abeydb.Database, error) {
	if n.config.DataDir == "" {
		return abeydb.NewMemDatabase(), nil
	}
	return abeydb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}

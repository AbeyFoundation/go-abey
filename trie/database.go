...
type Database struct {
	diskdb abeydb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb abeydb.Database) *Database {
...
func NewDatabaseWithCache(diskdb abeydb.Database, cache int) *Database {
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			...
			if batch.ValueSize() > abeydb.IdealBatchSize {
...
func (db *Database) commit(hash common.Hash, batch abeydb.Batch) error {
